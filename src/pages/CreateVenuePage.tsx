...
export function CreateVenuePage() {
  ...
  const blockedReason = useMemo(() => {
    if (!profile) return "You need to log in to create a venue.";
    if (!profile.venueManager) return "Only venue managers can create venues.";
    return null;
  }, [profile]);
  ...
  const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        isApiError(err) ? formatApiError(err) : "Failed to create venue.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <main className="w-full px-4 py-8 text-left sm:px-6 lg:px-8">
      {blockedReason ? (
        ...
        >
          <div>
            <h1 className="pb-0 text-gray-900">Create New Venue</h1>
            <p className="mt-1 text-sm text-gray-500">
              Add a new accommodation to your listings
            </p>
          </div>
